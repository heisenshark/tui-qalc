//code taken from cursive repo and modified the original license of that repo is MIT of course.
use std::ops::Deref;
use std::sync::Arc;
...
/// Provides access to the content of a [`AsyncTextView`].
///
/// [`AsyncTextView`]: struct.TextView.html
///
...
///
/// ```rust
/// # use cursive_core::views::{AsyncTextView, TextContent};
/// let mut content = TextContent::new("content");
/// let view = AsyncTextView::new_with_content(content.clone());
///
...
impl TextContent {
    ...
    /// Append `content` to the end of a `AsyncTextView`.
...
/// Internel representation of the content for a `AsyncTextView`.
///
...
/// # use cursive_core::views::AsyncTextView;
/// let mut siv = Cursive::new();
///
/// siv.add_layer(AsyncTextView::new("Hello world!"));
/// ```
